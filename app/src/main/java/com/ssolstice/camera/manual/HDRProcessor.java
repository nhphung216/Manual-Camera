...
public class HDRProcessor {
    ...
    private static class ResponseFunction {
        ...
        ResponseFunction(Context context, int id, List<Double> x_samples, List<Double> y_samples, List<Double> weights) {
            Logger.INSTANCE.d(TAG, "ResponseFunction");

            if (x_samples.size() != y_samples.size()) {
                Logger.INSTANCE.e(TAG, "unequal number of samples");
                ...
                Logger.INSTANCE.e(TAG, "not enough samples");
                // throw RuntimeException, as this is a programming error
                throw new RuntimeException();
            }
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "sum_wx = " + sum_wx);
                Logger.INSTANCE.d(TAG, "sum_wx2 = " + sum_wx2);
                Logger.INSTANCE.d(TAG, "sum_wxy = " + sum_wxy);
                Logger.INSTANCE.d(TAG, "sum_wy = " + sum_wy);
                Logger.INSTANCE.d(TAG, "sum_w = " + sum_w);
            }
            // need to solve:
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "A_numer = " + A_numer);
                Logger.INSTANCE.d(TAG, "A_denom = " + A_denom);
            }
            if (Math.abs(A_denom) < 1.0e-5) {
                Logger.INSTANCE.e(TAG, "denom too small");
                // will fall back to linear Y = AX
            } else {
                parameter_A = (float) (A_numer / A_denom);
                parameter_B = (float) ((sum_wy - parameter_A * sum_wx) / sum_w);
                if (MyDebug.LOG) {
                    Logger.INSTANCE.d(TAG, "parameter_A = " + parameter_A);
                    Logger.INSTANCE.d(TAG, "parameter_B = " + parameter_B);
                }
                // we don't want a function that is not monotonic, or can be negative!
                if (parameter_A < 1.0e-5) {
                    Logger.INSTANCE.e(TAG, "parameter A too small or negative: " + parameter_A);
                } else if (parameter_B < 1.0e-5) {
                    Logger.INSTANCE.e(TAG, "parameter B too small or negative: " + parameter_B);
                } else {
                    done = true;
                }
            }
            ...
            if (!done) {
                Logger.INSTANCE.e(TAG, "falling back to linear Y = AX");
                // linear Y = AX
                double numer = 0.0;
                double denom = 0.0;
                for (int i = 0; i < x_samples.size(); i++) {
                ...
                if (MyDebug.LOG) {
                    Logger.INSTANCE.d(TAG, "numer = " + numer);
                    Logger.INSTANCE.d(TAG, "denom = " + denom);
                }
            ...
            /*if( MyDebug.LOG ) {
                ...
                File file = new File(context.getExternalFilesDir(null).getPath() + "/com.ssolstice.camera.manual.hdr_samples_" + id + ".csv");
                if( file.exists() ) {
                    if( !file.delete() ) {
                        // keep FindBugs happy by checking return argument
                        Logger.INSTANCE.e(TAG, "failed to delete csv file");
                    }
                }
                ...
                try {
                    ...
                    for(int i=0;i<x_samples.size();i++) {
                        //Logger.INSTANCE.d(TAG, "log: " + i + " / " + x_samples.size());
                        double x = x_samples.get(i);
                        double y = y_samples.get(i);
                        double w = weights.get(i);
                    ...
                    Logger.INSTANCE.e(TAG, "failed to open csv file");
                    e.printStackTrace();
                }
                finally {
                    try {
                        ...
                        Logger.INSTANCE.e(TAG, "failed to close csv file");
                        e.printStackTrace();
                    }
                }
    ...
    public void processHDR(List<Bitmap> bitmaps, boolean release_bitmaps, Bitmap output_bitmap, boolean assume_sorted, SortCallback sort_cb, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, TonemappingAlgorithm tonemapping_algorithm, DROTonemappingAlgorithm dro_tonemapping_algorithm) throws HDRProcessorException {
        Logger.INSTANCE.d(TAG, "processHDR");
        if (!assume_sorted && !release_bitmaps) {
            Logger.INSTANCE.d(TAG, "take a copy of bitmaps array");
        ...
        if (n_bitmaps < 1 || n_bitmaps > 7) {
            Logger.INSTANCE.e(TAG, "n_bitmaps not supported: " + n_bitmaps);
    ...
    private ResponseFunction createFunctionFromBitmaps(int id, Bitmap in_bitmap, Bitmap out_bitmap, int offset_x, int offset_y) {
        Logger.INSTANCE.d(TAG, "createFunctionFromBitmaps");
        ...
        for (int y = 0; y < n_h_samples; y++) {
            ...
            for (int x = 0; x < n_w_samples; x++) {
                ...
                /*if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "sample response from " + x_coord + " , " + y_coord);*/
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "avg_in: " + avg_in);
            Logger.INSTANCE.d(TAG, "avg_out: " + avg_out);
            Logger.INSTANCE.d(TAG, "is_dark_exposure: " + is_dark_exposure);
        }
        {
            // calculate weights
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "min_value: " + min_value);
                Logger.INSTANCE.d(TAG, "max_value: " + max_value);
                Logger.INSTANCE.d(TAG, "med_value: " + med_value);
            }
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "min_value_y: " + min_value_y);
                Logger.INSTANCE.d(TAG, "max_value_y: " + max_value_y);
                Logger.INSTANCE.d(TAG, "med_value_y: " + med_value_y);
            }
            for (int i = 0; i < x_samples.size(); i++) {
                double value = x_samples.get(i);
    ...
    private void processHDRCore(List<Bitmap> bitmaps, boolean release_bitmaps, Bitmap output_bitmap, boolean assume_sorted, SortCallback sort_cb, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, TonemappingAlgorithm tonemapping_algorithm) {
        Logger.INSTANCE.d(TAG, "processHDRCore");
        ...
        if (use_renderscript) {
            initRenderscript();
            Logger.INSTANCE.d(TAG, "### time after creating renderscript: " + (System.currentTimeMillis() - time_s));
            // create allocations
            allocations = new Allocation[n_bitmaps];
            for (int i = 0; i < n_bitmaps; i++) {
                allocations[i] = Allocation.createFromBitmap(rs, bitmaps.get(i));
            }
            Logger.INSTANCE.d(TAG, "### time after creating allocations from bitmaps: " + (System.currentTimeMillis() - time_s));
        }
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "### time after autoAlignment: " + (System.currentTimeMillis() - time_s));
            Logger.INSTANCE.d(TAG, "median_brightness: " + median_brightness);
        }
        ...
        Logger.INSTANCE.d(TAG, "### time after creating response functions: " + (System.currentTimeMillis() - time_s));
        ...
        if (n_bitmaps % 2 == 0) {
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "remap for even number of images");
                Logger.INSTANCE.d(TAG, "    a: " + a);
                Logger.INSTANCE.d(TAG, "    b: " + b);
            }
            if (a < 1.0e-5f) {
            ...
            for (int i = 0; i < n_bitmaps; i++) {
                ...
                if (MyDebug.LOG) {
                    Logger.INSTANCE.d(TAG, "remapped: " + i);
                    Logger.INSTANCE.d(TAG, "    A: " + this_A + " -> " + response_functions[i].parameter_A);
                    Logger.INSTANCE.d(TAG, "    B: " + this_B + " -> " + response_functions[i].parameter_B);
                }
            }
        }
        ...
        for(int y=0;y<n_h_samples;y++) {
            ...
            for(int x=0;x<n_w_samples;x++) {
                ...
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "sample luminance from " + x_coord + " , " + y_coord);
        ...
        if( MyDebug.LOG )
            Logger.INSTANCE.d(TAG, "avg_luminance: " + avg_luminance);
        if( MyDebug.LOG )
            Logger.INSTANCE.d(TAG, "time after calculating average luminance: " + (System.currentTimeMillis() - time_s));
            */
        ...
        Logger.INSTANCE.d(TAG, "max_possible_value: " + max_possible_value);
        if (max_possible_value < 255.0f) {
            ...
            Logger.INSTANCE.d(TAG, "clamp max_possible_value to: " + max_possible_value);
        }
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "median_target: " + median_target);
            Logger.INSTANCE.d(TAG, "compare: " + 255.0f / max_possible_value);
            Logger.INSTANCE.d(TAG, "to: " + (((float) median_target) / (float) median_brightness + median_target / 255.0f - 1.0f));
        }
        if (255.0f / max_possible_value < ((float) median_target) / (float) median_brightness + median_target / 255.0f - 1.0f) {
            ...
            // much better compared to ManualCamera 1.39, though not quite as good as doing both this
            ...
            Logger.INSTANCE.d(TAG, "tonemap_denom: " + tonemap_denom);
            if (tonemap_denom != 0.0f) { // just in case
                tonemap_scale_c = (255.0f - median_target) / tonemap_denom;
                Logger.INSTANCE.d(TAG, "tonemap_scale_c (before setting min): " + tonemap_scale_c);
        ...
        Logger.INSTANCE.d(TAG, "tonemap_scale_c: " + tonemap_scale_c);
        ...
        switch (tonemapping_algorithm) {
            case TONEMAPALGORITHM_EXPONENTIAL: {
                ...
                Logger.INSTANCE.d(TAG, "linear_scale: " + linear_scale);
                break;
            }
            case TONEMAPALGORITHM_REINHARD: {
                // The basic algorithm is f(V) = V / (V+C), where V is the HDR value, C is tonemap_scale_c
                // This was used until ManualCamera 1.39, but has the problem of making images too dark: it
        ...
        if (!use_renderscript) {
            ...
            Logger.INSTANCE.d(TAG, "### time before HDRApplyFunction: " + (System.currentTimeMillis() - time_s));
            ...
            Logger.INSTANCE.d(TAG, "### time after HDRApplyFunction: " + (System.currentTimeMillis() - time_s));
        ...
        /*if( processHDRScript == null ) {
            ...
            Logger.INSTANCE.d(TAG, "call processHDRScript");
            ...
            Logger.INSTANCE.d(TAG, "### time before processHDRScript: " + (System.currentTimeMillis() - time_s));
            if (use_hdr_n) {
        ...
        /*processHDRScript.set_n_bitmaps_g(n_bitmaps);
        processHDRScript.forEach_hdr_n(allocations[base_bitmap], output_allocation);*/
            Logger.INSTANCE.d(TAG, "### time after processHDRScript: " + (System.currentTimeMillis() - time_s));

            if (release_bitmaps) {
                Logger.INSTANCE.d(TAG, "release bitmaps");
        ...
        freeScripts();
        Logger.INSTANCE.d(TAG, "### time for processHDRCore: " + (System.currentTimeMillis() - time_s));
    }
    ...
    private void processSingleImage(List<Bitmap> bitmaps, boolean release_bitmaps, Bitmap output_bitmap, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, DROTonemappingAlgorithm dro_tonemapping_algorithm) {
        Logger.INSTANCE.d(TAG, "processSingleImage");
        ...
        if (dro_tonemapping_algorithm == DROTonemappingAlgorithm.DROALGORITHM_GAINGAMMA) {
            ...
            Logger.INSTANCE.d(TAG, "### processSingleImage: time after computeHistogram: " + (System.currentTimeMillis() - time_s));
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "median brightness: " + brightness);
                Logger.INSTANCE.d(TAG, "max brightness: " + max_brightness);
            }
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "gain: " + gain);
                Logger.INSTANCE.d(TAG, "gamma: " + gamma);
                Logger.INSTANCE.d(TAG, "low_x: " + low_x);
                Logger.INSTANCE.d(TAG, "mid_x: " + mid_x);
            }

            if (Math.abs(gain - 1.0) > 1.0e-5 || max_brightness != 255 || Math.abs(gamma - 1.0) > 1.0e-5) {
                Logger.INSTANCE.d(TAG, "apply gain/gamma");
                //if( true )
                ...
                Logger.INSTANCE.d(TAG, "### processSingleImage: time after dro_brighten: " + (System.currentTimeMillis() - time_s));
            }
        }

        adjustHistogram(input_bitmap, output_bitmap, width, height, hdr_alpha, n_tiles, ce_preserve_blacks, time_s);

        Logger.INSTANCE.d(TAG, "### time for processSingleImage: " + (System.currentTimeMillis() - time_s));
    }
    ...
    private void processSingleImageRS(List<Bitmap> bitmaps, boolean release_bitmaps, Bitmap output_bitmap, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, DROTonemappingAlgorithm dro_tonemapping_algorithm) {
        ...
        initRenderscript();
        Logger.INSTANCE.d(TAG, "### processSingleImage: time after creating renderscript: " + (System.currentTimeMillis() - time_s));
        ...
        output_allocation.copyTo(output_bitmap);
        Logger.INSTANCE.d(TAG, "### processSingleImage: time after copying to bitmap: " + (System.currentTimeMillis() - time_s));
    ...
    void brightenImage(Bitmap bitmap, int brightness, int max_brightness, int brightness_target) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "brightenImage");
            Logger.INSTANCE.d(TAG, "brightness: " + brightness);
            Logger.INSTANCE.d(TAG, "max_brightness: " + max_brightness);
    ...
    public int getAvgSampleSize(int capture_result_iso, long capture_result_exposure_tim) {
        ...
        Logger.INSTANCE.d(TAG, "getAvgSampleSize: " + cached_avg_sample_size);
    ...
    public static class AvgData {
        ...
        public void destroy() {
            Logger.INSTANCE.d(TAG, "AvgData.destroy()");
    ...
    public AvgData processAvg(Bitmap bitmap_avg, Bitmap bitmap_new, float avg_factor, int iso, long exposure_time, float zoom_factor) throws HDRProcessorException {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "processAvg");
            Logger.INSTANCE.d(TAG, "avg_factor: " + avg_factor);
        }
        ...
        if( use_sharpness_test ) {
            ...
            if( sharpness_new > sharpness_avg ) {
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "use new image as reference");
            ...
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "sharp_index: " + sharp_index);
        }*/

        /*LuminanceInfo luminanceInfo = computeMedianLuminance(bitmap_avg, 0, 0, width, height);
        if( MyDebug.LOG )
            Logger.INSTANCE.d(TAG, "median: " + luminanceInfo.median_value);*/
        ...
        Logger.INSTANCE.d(TAG, "### time for processAvg: " + (System.currentTimeMillis() - time_s));
    ...
    public void updateAvg(AvgData avg_data, int width, int height, Bitmap bitmap_new, float avg_factor, int iso, long exposure_time, float zoom_factor) throws HDRProcessorException {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "updateAvg");
            Logger.INSTANCE.d(TAG, "avg_factor: " + avg_factor);
        }
        ...
        Logger.INSTANCE.d(TAG, "### time for updateAvg: " + (System.currentTimeMillis() - time_s));
    }
    ...
    private AvgData processAvgCore(AvgData avg_data, Bitmap bitmap_avg, Bitmap bitmap_new, int width, int height, float avg_factor, int iso, long exposure_time, float zoom_factor, long time_s) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "processAvgCore");
            Logger.INSTANCE.d(TAG, "iso: " + iso);
            Logger.INSTANCE.d(TAG, "zoom_factor: " + zoom_factor);
        }
        ...
        if (bitmap_avg != null && allocation_out == null && pixels_rgbf_out == null) {
            Logger.INSTANCE.d(TAG, "process first bitmap");
            floating_point = false;
        } else if (bitmap_avg == null && (allocation_out != null || pixels_rgbf_out != null)) {
            floating_point = true;
            Logger.INSTANCE.d(TAG, "processing existing result");
        } else {
...

        {
            ...
            Logger.INSTANCE.d(TAG, "scale_align_size: " + scale_align_size);
            boolean crop_to_centre = true;
            {
                // use scaled down and/or cropped bitmaps for alignment
                Logger.INSTANCE.d(TAG, "### time before creating allocations for autoalignment: " + (System.currentTimeMillis() - time_s));
                ...
                if (bitmap_avg_align == null) {
                    ...
                    Logger.INSTANCE.d(TAG, "### time after creating avg bitmap for autoalignment: " + (System.currentTimeMillis() - time_s));
                }
                ...
                Logger.INSTANCE.d(TAG, "### time after creating allocations for autoalignment: " + (System.currentTimeMillis() - time_s));
            }
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "avg_factor: " + avg_factor);
            Logger.INSTANCE.d(TAG, "tapered_wiener_scale: " + tapered_wiener_scale);
        }
        wiener_C /= tapered_wiener_scale;

        float wiener_C_cutoff = wiener_cutoff_factor * wiener_C;
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "wiener_C: " + wiener_C);
            Logger.INSTANCE.d(TAG, "wiener_cutoff_factor: " + wiener_cutoff_factor);
        }
        ...
        if (!use_renderscript /*&& !floating_point*/) {
            ...
            if( allocation_avg != null ) {
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "### time before AllocationToRGBf: " + (System.currentTimeMillis() - time_s));
                pixels_rgbf = AllocationToRGBf(allocation_avg, width, height);
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "### time after AllocationToRGBf: " + (System.currentTimeMillis() - time_s));
            }
            else {
                pixels_rgbf = new float[3*width*height];
            }*/
            if (pixels_rgbf_out == null) {
                Logger.INSTANCE.d(TAG, "need to create pixels_rgbf_out");
                pixels_rgbf_out = new float[3 * width * height];
                Logger.INSTANCE.d(TAG, "### time after create pixels_rgbf_out: " + (System.currentTimeMillis() - time_s));
            }
            ...
            Logger.INSTANCE.d(TAG, "### time after AvgApplyFunction: " + (System.currentTimeMillis() - time_s));
            /*RGBfToAllocation(pixels_rgbf, allocation_out, width, height);
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "### time after RGBfToAllocation: " + (System.currentTimeMillis() - time_s));*/
            ...
            /*if( allocation_orig == null ) {
                ...
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "create allocation_orig");
            ...
            if (allocation_out == null) {
                Logger.INSTANCE.d(TAG, "need to create allocation_out");
                allocation_out = Allocation.createTyped(rs, Type.createXY(rs, Element.F32_3(rs), width, height));
                Logger.INSTANCE.d(TAG, "### time after create allocation_out: " + (System.currentTimeMillis() - time_s));
            }
            ...
            if (allocation_avg == null) {
                ...
                Logger.INSTANCE.d(TAG, "### time after creating allocation_avg from bitmap: " + (System.currentTimeMillis() - time_s));
            }
        ...
        if( first && separate_first_pass ) {
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "### time before convert_to_f: " + (System.currentTimeMillis() - time_s));
            processAvgScript.forEach_convert_to_f(allocation_avg, allocation_out);
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "### time after convert_to_f: " + (System.currentTimeMillis() - time_s));
            if( free_allocation_avg ) {
                allocation_avg.destroy();
                free_allocation_avg = false;
            }
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "release bitmap_avg");
            //bitmap_avg.recycle();
            //bitmap_avg = null;
            ...
            Allocation allocation_new = Allocation.createFromBitmap(rs, bitmap_new);
            Logger.INSTANCE.d(TAG, "### time after creating allocation_new from bitmap: " + (System.currentTimeMillis() - time_s));
            ...
            if (allocation_orig == null) {
                ...
                Logger.INSTANCE.d(TAG, "create allocation_avg");
                allocation_orig = Allocation.createFromBitmap(rs, bitmap_avg);
            }
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "allocation_orig: " + allocation_orig);
            }
        ...
        if( MyDebug.LOG ) {
            Logger.INSTANCE.d(TAG, "max_weight: " + max_weight);
        }
        processAvgScript.set_max_weight(max_weight);*/

            Logger.INSTANCE.d(TAG, "call processAvgScript");
            Logger.INSTANCE.d(TAG, "### time before processAvgScript: " + (System.currentTimeMillis() - time_s));
            if (floating_point)
                processAvgScript.forEach_avg_f(allocation_avg, allocation_out);
            else
                processAvgScript.forEach_avg(allocation_avg, allocation_out);
            Logger.INSTANCE.d(TAG, "### time after processAvgScript: " + (System.currentTimeMillis() - time_s));
        ...
        if (bitmap_new != null) {
            Logger.INSTANCE.d(TAG, "release bitmap_new");
            bitmap_new.recycle();
            //noinspection UnusedAssignment
            bitmap_new = null;
        }

        Logger.INSTANCE.d(TAG, "### time for processAvgCore: " + (System.currentTimeMillis() - time_s));
    ...
    public void processAvgMulti(List<Bitmap> bitmaps, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks) throws HDRProcessorException {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "processAvgMulti");
        ...
        Logger.INSTANCE.d(TAG, "### time for processAvgMulti: " + (System.currentTimeMillis() - time_s));
    }
    ...
    void autoAlignment(int[] offsets_x, int[] offsets_y, int width, int height, List<Bitmap> bitmaps, int base_bitmap, boolean use_mtb, int max_align_scale) {
        Logger.INSTANCE.d(TAG, "autoAlignment");
        initRenderscript();
        Allocation[] allocations = null;
        if (use_renderscript) {
    ...
    private BrightnessDetails autoAlignment(int[] offsets_x, int[] offsets_y, Allocation[] allocations, int width, int height, List<Bitmap> bitmaps, int base_bitmap, boolean assume_sorted, SortCallback sort_cb, boolean use_mtb, int min_step_size, boolean crop_to_centre, int max_align_scale, int full_width, int full_height, long time_s) {
        if (MyDebug.LOG) {
            ...
            Logger.INSTANCE.d(TAG, "use_mtb: " + use_mtb);
            Logger.INSTANCE.d(TAG, "max_align_scale: " + max_align_scale);
            Logger.INSTANCE.d(TAG, "bitmaps: " + bitmaps.size());
            if (allocations != null) {
                Logger.INSTANCE.d(TAG, "allocations: " + allocations.length);
                for (Allocation allocation : allocations) {
                    Logger.INSTANCE.d(TAG, "    allocation:");
                    Logger.INSTANCE.d(TAG, "    element: " + allocation.getElement());
                    Logger.INSTANCE.d(TAG, "    type X: " + allocation.getType().getX());
                    Logger.INSTANCE.d(TAG, "    type Y: " + allocation.getType().getY());
                }
            }
        }
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "mtb_x: " + mtb_x);
            Logger.INSTANCE.d(TAG, "mtb_y: " + mtb_y);
            Logger.INSTANCE.d(TAG, "mtb_width: " + mtb_width);
            Logger.INSTANCE.d(TAG, "mtb_height: " + mtb_height);
        }
        ...
        if (use_mtb) {
            ...
            for (int i = 0; i < n_images; i++) {
                ...
                Logger.INSTANCE.d(TAG, i + ": median_value: " + luminanceInfos[i].median_value);
            }
            Logger.INSTANCE.d(TAG, "time after computeMedianLuminance: " + (System.currentTimeMillis() - time_s));
        }

        if (!assume_sorted && use_mtb) {
            Logger.INSTANCE.d(TAG, "sort bitmaps");
            class BitmapInfo {
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "before sorting:");
                for (int i = 0; i < n_images; i++) {
                    Logger.INSTANCE.d(TAG, "    " + i + ": " + luminanceInfos[i]);
                }
            }
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "after sorting:");
                for (int i = 0; i < n_images; i++) {
                    Logger.INSTANCE.d(TAG, "    " + i + ": " + luminanceInfos[i]);
                }
            }
            if (sort_cb != null) {
                ...
                Logger.INSTANCE.d(TAG, "sort_order: " + sort_order);
                sort_cb.sortOrder(sort_order);
            }
        }
        /*{
        ...
        if (!use_renderscript) {
            ...
            Logger.INSTANCE.d(TAG, "### time after creating mtb_allocations: " + (System.currentTimeMillis() - time_s));

            // create RenderScript
            if (createMTBScript == null) {
                createMTBScript = new ScriptC_create_mtb(rs);
                Logger.INSTANCE.d(TAG, "### time after creating createMTBScript: " + (System.currentTimeMillis() - time_s));
            }
        ...
        for (int i = 0; i < n_images; i++) {
            ...
            if (use_mtb) {
                ...
                Logger.INSTANCE.d(TAG, i + ": median_value: " + median_value);

                /*if( median_value < 16 ) {
                    // needed for testHDR2, testHDR28
                    if( MyDebug.LOG )
                        Logger.INSTANCE.d(TAG, "image too dark to do alignment");
                    if( mtb_bitmaps != null )
                        mtb_bitmaps[i] = null;
            ...
            if (use_mtb && luminanceInfos[i].noisy) {
                Logger.INSTANCE.d(TAG, "unable to compute median luminance safely");
                if (mtb_bitmaps != null)
                    mtb_bitmaps[i] = null;
                if (mtb_allocations != null)
                    mtb_allocations[i] = null;
            ...
            Logger.INSTANCE.d(TAG, i + ": median_value is now: " + median_value);
            ...
            if (!use_renderscript) {
                ...
                Logger.INSTANCE.d(TAG, "### time after CreateMTBApplyFunction: " + (System.currentTimeMillis() - time_s));
                ...
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "### time after copying to mtb_allocations: " + (System.currentTimeMillis() - time_s));*/
                ...
                Logger.INSTANCE.d(TAG, "call createMTBScript");
                ...
                Logger.INSTANCE.d(TAG, "time after createMTBScript: " + (System.currentTimeMillis() - time_s));
        ...
        Logger.INSTANCE.d(TAG, "### time after all createMTBScript: " + (System.currentTimeMillis() - time_s));
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "max_dim: " + max_dim);
            Logger.INSTANCE.d(TAG, "max_ideal_size: " + max_ideal_size);
            Logger.INSTANCE.d(TAG, "initial_step_size: " + initial_step_size);
        }

        if (mtb_bitmaps != null && mtb_bitmaps[base_bitmap] == null) {
            Logger.INSTANCE.d(TAG, "base image not suitable for image alignment");
        ...
        for (int i = 0; i < n_images; i++) {
            ...
            if (mtb_bitmaps != null && mtb_bitmaps[i] == null) {
                Logger.INSTANCE.d(TAG, "image " + i + " not suitable for image alignment");
                continue;
            }
            ...
            while (step_size > min_step_size) {
                ...
                if (MyDebug.LOG) {
                    Logger.INSTANCE.d(TAG, "call alignMTBScript for image: " + i);
                    Logger.INSTANCE.d(TAG, "    versus base image: " + base_bitmap);
                    Logger.INSTANCE.d(TAG, "step_size: " + step_size);
                    Logger.INSTANCE.d(TAG, "pixel_step_size: " + pixel_step_size);
                }
                ...
                if (!use_renderscript) {
                    ...
                    Logger.INSTANCE.d(TAG, "### time after AlignMTBApplyFunction: " + (System.currentTimeMillis() - time_s));
                ...
                /*if( use_pyramid ) {
                    ...
                    for(int j=0;j<2;j++) {
                        ...
                        if( MyDebug.LOG ) {
                            Logger.INSTANCE.d(TAG, "create scaled image: " + j);
                            Logger.INSTANCE.d(TAG, "    scaled_width: " + scaled_width);
                            Logger.INSTANCE.d(TAG, "    scaled_height: " + scaled_height);
                        }
                    ...
                    if( MyDebug.LOG ) {
                        Logger.INSTANCE.d(TAG, "off_x: " + off_x);
                        Logger.INSTANCE.d(TAG, "off_y: " + off_y);
                    }
                    ...
                    if (MyDebug.LOG) {
                        Logger.INSTANCE.d(TAG, "time for alignMTBScript: " + (System.currentTimeMillis() - this_time_s));
                        Logger.INSTANCE.d(TAG, "### time after alignMTBScript: " + (System.currentTimeMillis() - time_s));
                    }
                    errors = new int[9];
                ...
                for (int j = 0; j < 9; j++) {
                    ...
                    Logger.INSTANCE.d(TAG, "    errors[" + j + "]: " + this_error);
                ...
                /*if( best_id != 4 ) {
                    ...
                    for(int j=0;j<9;j++) {
                        ...
                        if( this_off_x * that_off_x == -1 || this_off_y * that_off_y == -1 ) {
                            ...
                            if( MyDebug.LOG )
                                Logger.INSTANCE.d(TAG, "    opposite errors[" + j + "] diff: " + diff);
                            if( Math.abs(diff) <= 0.02f ) {
                                if( MyDebug.LOG )
                                    Logger.INSTANCE.d(TAG, "    reject auto-alignment");
                                best_id = 4;
                                break;
                            }
                        }
                ...
                if (best_id != -1) {
                    ...
                    this_off_y--;
                    if (MyDebug.LOG) {
                        Logger.INSTANCE.d(TAG, "this_off_x: " + this_off_x);
                        Logger.INSTANCE.d(TAG, "this_off_y: " + this_off_y);
                    }
                    offsets_x[i] += this_off_x * step_size;
                    offsets_y[i] += this_off_y * step_size;
                    if (MyDebug.LOG) {
                        Logger.INSTANCE.d(TAG, "offsets_x is now: " + offsets_x[i]);
                        Logger.INSTANCE.d(TAG, "offsets_y is now: " + offsets_y[i]);
                    }
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "resultant offsets for image: " + i);
                Logger.INSTANCE.d(TAG, "resultant offsets_x: " + offsets_x[i]);
                Logger.INSTANCE.d(TAG, "resultant offsets_y: " + offsets_y[i]);
            }
        }
    ...
    private LuminanceInfo computeMedianLuminance(Bitmap bitmap, int mtb_x, int mtb_y, int mtb_width, int mtb_height) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "computeMedianLuminance");
            Logger.INSTANCE.d(TAG, "mtb_x: " + mtb_x);
            Logger.INSTANCE.d(TAG, "mtb_y: " + mtb_y);
            Logger.INSTANCE.d(TAG, "mtb_width: " + mtb_width);
            Logger.INSTANCE.d(TAG, "mtb_height: " + mtb_height);
        ...
        for (int y = 0; y < n_h_samples; y++) {
            ...
            for (int x = 0; x < n_w_samples; x++) {
                ...
                /*if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "sample value from " + x_coord + " , " + y_coord);*/
        ...
        for (int i = 255; i >= 0; i--) {
            /*if( histo[i] > 0 ) {
                if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "max luminance " + i);
                max_value = i;
                break;
            }*/
            count += histo[i];
            if (count >= total / 10) {
                Logger.INSTANCE.d(TAG, "hi luminance " + i);
                hi_value = i;
                break;
            }
        }
        ...
        for (int i = 0; i < 256; i++) {
            ...
            if (min_value == -1 && histo[i] > 0) {
                Logger.INSTANCE.d(TAG, "min luminance " + i);
                min_value = i;
            }
            if (count >= middle) {
                Logger.INSTANCE.d(TAG, "median luminance " + i);
                ...
                if (MyDebug.LOG) {
                    ...
                    Logger.INSTANCE.d(TAG, "count: " + count);
                    Logger.INSTANCE.d(TAG, "n_below: " + n_below);
                    Logger.INSTANCE.d(TAG, "n_above: " + n_above);
                    Logger.INSTANCE.d(TAG, "frac_below: " + frac_below);
                    Logger.INSTANCE.d(TAG, "frac_above: " + frac_above);
                }
                if (frac_below < 0.2) {
                    // needed for testHDR2, testHDR28
                    // note that we don't exclude cases where frac_above is too small, as this could be an overexposed image - see testHDR31
                    Logger.INSTANCE.d(TAG, "too dark/noisy");
                    noisy = true;
                }
    ...
    void clipHistogram(int[] histogram, int[] temp_c_histogram, int sub_width, int sub_height, boolean ce_preserve_blacks) {
        ...
        int clip_limit = (5 * n_pixels) / 256;
                    /*if( MyDebug.LOG ) {
                        Logger.INSTANCE.d(TAG, "clip_limit: " + clip_limit);
                        Logger.INSTANCE.d(TAG, "    relative clip limit: " + clip_limit*256.0f/n_pixels);
                    }*/
        {
            ...
            clip_limit = (top + bottom) / 2;
                        /*if( MyDebug.LOG ) {
                            Logger.INSTANCE.d(TAG, "updated clip_limit: " + clip_limit);
                            Logger.INSTANCE.d(TAG, "    relative updated clip limit: " + clip_limit*256.0f/n_pixels);
                        }*/
        }
        int n_clipped = 0;
        for (int x = 0; x < 256; x++) {
            if (histogram[x] > clip_limit) {
                            /*if( MyDebug.LOG ) {
                                Logger.INSTANCE.d(TAG, "    " + x + " : " + histogram[x] + " : " + (histogram[x]*256.0f/n_pixels));
                            }*/
        ...
        int n_clipped_per_bucket = n_clipped / 256;
                        /*if( MyDebug.LOG ) {
                            Logger.INSTANCE.d(TAG, "n_clipped: " + n_clipped);
                            Logger.INSTANCE.d(TAG, "n_clipped_per_bucket: " + n_clipped_per_bucket);
                        }*/
        for (int x = 0; x < 256; x++) {
        ...
        if (ce_preserve_blacks) {
            ...
            //   the cumulative histogram (as computed before modifying any values).
                        /*if( MyDebug.LOG ) {
                            for(int x=0;x<256;x++) {
                                Logger.INSTANCE.d(TAG, "pre-brighten histogram[" + x + "] = " + histogram[x]);
                            }
                        }*/
            ...
            Logger.INSTANCE.d(TAG, "equal_limit: " + equal_limit);
            ...
            for (int x = 0; x < dark_threshold_c; x++) {
                ...
                /*if( MyDebug.LOG )
                    Logger.INSTANCE.d(TAG, "x: " + x + " ; limit: " + limit);*/
                            /*histogram[x] = Math.max(histogram[x], limit);
                            if( MyDebug.LOG )
                                Logger.INSTANCE.d(TAG, "    histogram pulled up to: "  + histogram[x]);*/
    ...
    void adjustHistogram(Bitmap bitmap_in, Bitmap bitmap_out, int width, int height, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, long time_s) {
        Logger.INSTANCE.d(TAG, "adjustHistogram [bitmap]");
        ...
        if (adjust_histogram_local) {
            ...
            Logger.INSTANCE.d(TAG, "time before creating histograms: " + (System.currentTimeMillis() - time_s));
            // create histograms
            ...
            for (int i = 0; i < n_tiles; i++) {
                ...
                for (int j = 0; j < n_tiles; j++) {
                    ...
                    if (stop_y == start_y)
                        continue;
                        /*if( MyDebug.LOG )
                            Logger.INSTANCE.d(TAG, i + " , " + j + " : " + start_x + " , " + start_y + " to " + stop_x + " , " + stop_y);*/
                    ...
                    /*if( MyDebug.LOG ) {
                        for(int x=0;x<256;x++) {
                            Logger.INSTANCE.d(TAG, "histogram[" + x + "] = " + histogram[x] + " cumulative: " + c_histogram[histogram_offset+x]);
                        }
                    }*/
                }
            }

            Logger.INSTANCE.d(TAG, "adjustHistogram: time after creating histograms: " + (System.currentTimeMillis() - time_s));
            ...
            Logger.INSTANCE.d(TAG, "time after adjusting histogram: " + (System.currentTimeMillis() - time_s));
        }
    }
    ...
    void adjustHistogramRS(Allocation allocation_in, Allocation allocation_out, int width, int height, float hdr_alpha, int n_tiles, boolean ce_preserve_blacks, long time_s) {
        Logger.INSTANCE.d(TAG, "adjustHistogram [renderscript]");
        ...
        if (adjust_histogram_local) {
                ...
                for (int j = 0; j < n_tiles; j++) {
                    ...
                    histogramAllocation.copyTo(histogram);
                        ...
                        /*if( MyDebug.LOG ) {
                            ...
                            for(int x=0;x<256;x++) {
                                Logger.INSTANCE.d(TAG, "histogram[" + x + "] = " + histogram[x] + " debug_histogram: " + debug_histogram[x]);
            ...
            Logger.INSTANCE.d(TAG, "time before histogramAdjustScript: " + (System.currentTimeMillis() - time_s));
            histogramAdjustScript.forEach_histogram_adjust(allocation_in, allocation_out);
            Logger.INSTANCE.d(TAG, "time after histogramAdjustScript: " + (System.currentTimeMillis() - time_s));
    ...
    private Allocation computeHistogramAllocation(Allocation allocation_in, boolean avg, boolean floating_point, long time_s) {
        Logger.INSTANCE.d(TAG, "computeHistogramAllocation");
        Allocation histogramAllocation = Allocation.createSized(rs, Element.I32(rs), 256);
        ...
        if (use_custom_histogram) {
            ...
            Logger.INSTANCE.d(TAG, "time before histogramScript: " + (System.currentTimeMillis() - time_s));
            if (avg) {
                if (floating_point)
            ...
            Logger.INSTANCE.d(TAG, "time after histogramScript: " + (System.currentTimeMillis() - time_s));
        } else {
            ...
            Logger.INSTANCE.d(TAG, "call histogramScriptIntrinsic");
    ...
    public int[] computeHistogram(Bitmap bitmap, HistogramType type) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "computeHistogram [bitmap]");
        ...
        if (!use_renderscript) {
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "image size: " + bitmap.getWidth() + " x " + bitmap.getHeight());
                Logger.INSTANCE.d(TAG, "### time to compute histogram: " + (System.currentTimeMillis() - time_s));
            }
            return histogram;
        }
        initRenderscript();
        Allocation allocation_in = Allocation.createFromBitmap(rs, bitmap);
        Logger.INSTANCE.d(TAG, "time after createFromBitmap: " + (System.currentTimeMillis() - time_s));
        boolean avg;
        switch (type) {
            case HISTOGRAM_TYPE_RGB:
            case HISTOGRAM_TYPE_LUMINANCE:
    ...
    private int[] computeHistogram(float[] pixels, int width, int height, boolean avg) {
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "image size: " + width + " x " + height);
            Logger.INSTANCE.d(TAG, "### time to compute histogram: " + (System.currentTimeMillis() - time_s));
        }
        return histogram;
    }
    ...
    private int[] computeHistogramRS(Allocation allocation, int width, int height, boolean avg, boolean floating_point) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "computeHistogram [renderscript/allocation]");
            Logger.INSTANCE.d(TAG, "avg: " + avg);
            Logger.INSTANCE.d(TAG, "floating_point: " + floating_point);
        }
        long time_s = System.currentTimeMillis();
        /*if( !use_renderscript && !avg ) {
            ...
            if( MyDebug.LOG ) {
                Logger.INSTANCE.d(TAG, "allocation size: " + width + " x " + height);
                Logger.INSTANCE.d(TAG, "### time to compute histogram: " + (System.currentTimeMillis() - time_s));
            }
            return histogram;
        }*/
    ...
    private static int getBrightnessTarget(int brightness, float max_gain_factor, int ideal_brightness) {
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "brightness: " + brightness);
            Logger.INSTANCE.d(TAG, "max_gain_factor: " + max_gain_factor);
            Logger.INSTANCE.d(TAG, "ideal_brightness: " + ideal_brightness);
        }
    ...
    public static BrightenFactors computeBrightenFactors(boolean has_iso_exposure, int iso, long exposure_time, int brightness, int max_brightness) {
        ...
        if (MyDebug.LOG) {
            ...
            Logger.INSTANCE.d(TAG, "max_brightness: " + max_brightness);
            Logger.INSTANCE.d(TAG, "ideal_brightness: " + ideal_brightness);
            Logger.INSTANCE.d(TAG, "brightness target: " + brightness_target);
            //Logger.INSTANCE.d(TAG, "max target: " + max_target);
        }
    ...
    private static BrightenFactors computeBrightenFactors(boolean has_iso_exposure, int iso, long exposure_time, int brightness, int max_brightness, int brightness_target, boolean brighten_only) {
        ...
        if (gain < 1.0f && brighten_only) {
            gain = 1.0f;
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "clamped gain to: " + gain);
            }
        }
        float gamma = 1.0f;
        ...
        /*if( max_possible_value > 255.0f ) {
            ...
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "limit gain to: " + gain);
        ...
        if( gamma > 1.0f ) {
            gamma = 1.0f;
            if( MyDebug.LOG ) {
                Logger.INSTANCE.d(TAG, "clamped gamma to : " + gamma);
            }
        }
        ...
        float mid_x = 255.5f;
        if (max_possible_value > 255.0f) {
            Logger.INSTANCE.d(TAG, "use piecewise gain/gamma");
            ...
            Logger.INSTANCE.d(TAG, "alt_gain: " + alt_gain);
            if (alt_gain > gain) {
                gain = alt_gain;
                Logger.INSTANCE.d(TAG, "increased gain to: " + gain);
            }
        }
        float low_x = 0.0f;
    ...
    private float computeBlackLevel(HistogramInfo histogramInfo, int[] histo, int iso) {
        float black_level = 0.0f;
        {
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "percentile: " + percentile);
                Logger.INSTANCE.d(TAG, "darkest_brightness: " + darkest_brightness);
                Logger.INSTANCE.d(TAG, "black_level is now: " + black_level);
            }
        }
        return black_level;
    }
    ...
    private Bitmap avgBrightenRGBf(float[] pixels_in_rgbf, int width, int height, int iso, long exposure_time) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "avgBrightenRGBf");
            Logger.INSTANCE.d(TAG, "iso: " + iso);
        ...
        int brightness = histogramInfo.median_brightness;
        int max_brightness = histogramInfo.max_brightness;
        Logger.INSTANCE.d(TAG, "### time after computeHistogram: " + (System.currentTimeMillis() - time_s));

        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "median brightness: " + histogramInfo.median_brightness);
            Logger.INSTANCE.d(TAG, "mean brightness: " + histogramInfo.mean_brightness);
            Logger.INSTANCE.d(TAG, "max brightness: " + max_brightness);
            /*for(int i=0;i<256;i++) {
                Logger.INSTANCE.d(TAG, "histogram[" + i + "]: " + histo[i]);
            }*/
        }
        ...
        if( MyDebug.LOG ) {
            Logger.INSTANCE.d(TAG, "gamma " + gamma);
            Logger.INSTANCE.d(TAG, "gain " + gain);
            Logger.INSTANCE.d(TAG, "gain2 " + max_target / ((float)Math.pow(max_brightness/255.0f, gamma) * 255.0f));
        }*/
        /*float gain = brightness_target / (float)brightness;
        if( MyDebug.LOG ) {
            Logger.INSTANCE.d(TAG, "gain: " + gain);
        ...
        Logger.INSTANCE.d(TAG, "median_filter_strength: " + median_filter_strength);
        ...
        Logger.INSTANCE.d(TAG, "### time after AvgBrightenApplyFunction: " + (System.currentTimeMillis() - time_s));
        ...
        if (iso < 1100 && exposure_time < 1000000000L / 59) {
            ...
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "dro alpha: " + alpha);
                Logger.INSTANCE.d(TAG, "dro amount: " + amount);
            }
            adjustHistogram(output_bitmap, output_bitmap, width, height, amount, 1, true, time_s);
            Logger.INSTANCE.d(TAG, "### time after adjustHistogram: " + (System.currentTimeMillis() - time_s));
        }

        Logger.INSTANCE.d(TAG, "### total time for avgBrighten: " + (System.currentTimeMillis() - time_s));
    ...
    private Bitmap avgBrightenRS(Allocation input, int width, int height, int iso, long exposure_time) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "avgBrightenRS");
            Logger.INSTANCE.d(TAG, "iso: " + iso);
            Logger.INSTANCE.d(TAG, "exposure_time: " + exposure_time);
        }
        initRenderscript();
        ...
        Logger.INSTANCE.d(TAG, "### time after avg_brighten: " + (System.currentTimeMillis() - time_s));
    ...
    private float computeSharpness(Allocation allocation_in, int width, long time_s) {
        Logger.INSTANCE.d(TAG, "computeSharpness");
        Logger.INSTANCE.d(TAG, "### time: " + (System.currentTimeMillis() - time_s));
        Allocation sumsAllocation = Allocation.createSized(rs, Element.I32(rs), width);
        Logger.INSTANCE.d(TAG, "### time after createSized: " + (System.currentTimeMillis() - time_s));
        /*if( sharpnessScript == null ) {
            sharpnessScript = new ScriptC_calculate_sharpness(rs);
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "### time after create sharpnessScript: " + (System.currentTimeMillis() - time_s));
        }*/
        ScriptC_calculate_sharpness sharpnessScript = new ScriptC_calculate_sharpness(rs);
        Logger.INSTANCE.d(TAG, "### time after create sharpnessScript: " + (System.currentTimeMillis() - time_s));
        Logger.INSTANCE.d(TAG, "bind sums allocation");
        ...
        Logger.INSTANCE.d(TAG, "call sharpnessScript");
        Logger.INSTANCE.d(TAG, "### time before sharpnessScript: " + (System.currentTimeMillis() - time_s));
        sharpnessScript.forEach_calculate_sharpness(allocation_in);
        Logger.INSTANCE.d(TAG, "### time after sharpnessScript: " + (System.currentTimeMillis() - time_s));
        ...
        for (int i = 0; i < width; i++) {
            /*if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "sums[" + i + "] = " + sums[i]);*/
            total_sum += (float) sums[i];
        }
        Logger.INSTANCE.d(TAG, "total_sum: " + total_sum);
        return total_sum;
    }
}
