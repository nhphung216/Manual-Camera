...
public class SoundPoolManager {
    ...
    void initSound() {
        if (sound_pool == null) {
            Logger.INSTANCE.d(TAG, "create new sound_pool");
            {
    ...
    void releaseSound() {
        if (sound_pool != null) {
            Logger.INSTANCE.d(TAG, "release sound_pool");
            sound_pool.release();
            sound_pool = null;
            sound_ids = null;
        }
    ...
    void loadSound(int resource_id) {
        if (sound_pool != null) {
            Logger.INSTANCE.d(TAG, "loading sound resource: " + resource_id);
            int sound_id = sound_pool.load(context, resource_id, 1);
            Logger.INSTANCE.d(TAG, "    loaded sound: " + sound_id);
    ...
    void playSound(int resource_id) {
        if (sound_pool != null) {
            if (sound_ids.indexOfKey(resource_id) < 0) {
                Logger.INSTANCE.d(TAG, "resource not loaded: " + resource_id);
            } else {
                int sound_id = sound_ids.get(resource_id);
                Logger.INSTANCE.d(TAG, "play sound: " + sound_id);
