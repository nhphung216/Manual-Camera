...
public class SaveLocationHistory {
    ...
    SaveLocationHistory(MainActivity main_activity, String pref_base, String folder_name) {
        Logger.INSTANCE.d(TAG, "pref_base: " + pref_base);
        ...
        Logger.INSTANCE.d(TAG, "save_location_history_size: " + save_location_history_size);
        for (int i = 0; i < save_location_history_size; i++) {
            String string = sharedPreferences.getString(pref_base + "_" + i, null);
            if (string != null) {
                Logger.INSTANCE.d(TAG, "save_location_history " + i + ": " + string);
    ...
    private void updateFolderHistory(String folder_name) {
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "updateFolderHistory: " + folder_name);
            Logger.INSTANCE.d(TAG, "save_location_history size: " + save_location_history.size());
            for (int i = 0; i < save_location_history.size(); i++) {
                Logger.INSTANCE.d(TAG, save_location_history.get(i));
            }
        }
        ...
        writeSaveLocations();
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "updateFolderHistory exit:");
            Logger.INSTANCE.d(TAG, "save_location_history size: " + save_location_history.size());
            for (int i = 0; i < save_location_history.size(); i++) {
                Logger.INSTANCE.d(TAG, save_location_history.get(i));
            }
    ...
    void clearFolderHistory(String folder_name) {
        Logger.INSTANCE.d(TAG, "clearFolderHistory: " + folder_name);
        save_location_history.clear();
    ...
    private void writeSaveLocations() {
        Logger.INSTANCE.d(TAG, "writeSaveLocations");
        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(main_activity);
        SharedPreferences.Editor editor = sharedPreferences.edit();
        editor.putInt(pref_base + "_size", save_location_history.size());
        Logger.INSTANCE.d(TAG, "save_location_history_size = " + save_location_history.size());
        for (int i = 0; i < save_location_history.size(); i++) {
