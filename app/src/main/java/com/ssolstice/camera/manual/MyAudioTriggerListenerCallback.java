...
public class MyAudioTriggerListenerCallback implements AudioListener.AudioListenerCallback {
    ...
    public void onAudio(int level) {
        ...
        /*if( level > 150 ) {
            if( MyDebug.LOG )
                Logger.INSTANCE.d(TAG, "loud noise!: " + level);
            audio_trigger = true;
        }*/
        ...
        if (MyDebug.LOG) {
            Logger.INSTANCE.d(TAG, "noise_sensitivity: " + audio_noise_sensitivity);
            Logger.INSTANCE.d(TAG, "diff: " + diff);
        }

        if (diff > audio_noise_sensitivity) {
            Logger.INSTANCE.d(TAG, "got louder!: " + last_level + " to " + level + " , diff: " + diff);
            time_quiet_loud = System.currentTimeMillis();
            Logger.INSTANCE.d(TAG, "    time: " + time_quiet_loud);
        } else if (diff < -audio_noise_sensitivity && time_quiet_loud != -1) {
            Logger.INSTANCE.d(TAG, "got quieter!: " + last_level + " to " + level + " , diff: " + diff);
            long time_now = System.currentTimeMillis();
            long duration = time_now - time_quiet_loud;
            if (MyDebug.LOG) {
                Logger.INSTANCE.d(TAG, "stopped being loud - was loud since: " + time_quiet_loud);
                Logger.INSTANCE.d(TAG, "    time_now: " + time_now);
                Logger.INSTANCE.d(TAG, "    duration: " + duration);
            }
            if (duration < 1500) {
                Logger.INSTANCE.d(TAG, "audio_trigger set");
                audio_trigger = true;
            }
            time_quiet_loud = -1;
        } else {
            Logger.INSTANCE.d(TAG, "audio level: " + last_level + " to " + level + " , diff: " + diff);
        }

        last_level = level;

        if (audio_trigger) {
            Logger.INSTANCE.d(TAG, "audio trigger");
            ...
            if (time_last_audio_trigger_photo != -1 && time_now - time_last_audio_trigger_photo < 5000) {
                ...
                Logger.INSTANCE.d(TAG, "ignore loud noise due to too soon since last audio triggered photo: " + (time_now - time_last_audio_trigger_photo));
                ...
                Logger.INSTANCE.d(TAG, "ignore loud noise due to audio listener option turned off");
            } else {
                Logger.INSTANCE.d(TAG, "audio trigger from loud noise");
